...
public class PacmanGame {
    ...
    public void render(double timestamp, Heading input) {
	this.buffer.beginDraw();
	this.buffer.background(0);
	this.buffer.image(board.pg, 0, 0);
	advanceGame(timestamp, input);
    ...
    void setElapsed(double timestamp) {
	if (startTimestamp > STANDSTILL_MILLIS) {
	    gameTimestamp = timestamp - startTimestamp;
	}
    }

    void advanceGame(double timestamp, Heading input) {
	if (startTimestamp == 0) {
	    startTimestamp = timestamp + STANDSTILL_MILLIS;
	    return;
	}
	...
	for (GhostSprite ghost : ghosts) {
	    for (int ticks = ghost.ticksNeeded(gameTimestamp);
    ...
    float pacPeriod() {
        if (fastUntil > 0 && gameTimestamp < fastUntil) {
            return fastPeriod;
        }
        return normalPeriod;
    }
	...
	if (this.lastElapsed <= 0) {
	    this.lastElapsed = gameTimestamp;
	    return 0;
	}
	double delta = this.gameTimestamp - this.lastElapsed;
	float p = pacPeriod();
	float r = BLOCK_PIXELS / p;

	this.lastElapsed = this.gameTimestamp;
